<template>
  <div class="sfcConditionNodeContainer" :class="{ vertical: vertical }">
    <div class="sfcConditionAddBtn" v-if="!readonly" @click="onAddConditionBranchClick">添加条件</div>
    <div class="sfcConditionNodeItemList">
      <div class="sfcConditionNodeItem" v-for="node in data.children" :key="node.id">
        <!-- 左侧的竖线 -->
        <div class="sfcConditionNodeItemLine sfcConditionNodeItemFirstLine"></div>
        <!-- 右侧的竖线 -->
        <div v-if="!data.isEnd" class="sfcConditionNodeItemLine sfcConditionNodeItemLastLine"></div>
        <!-- 连接竖线和节点的水平线 -->
        <div class="sfcConditionNodeItemLinkLine"></div>
        <div class="sfcConditionNodeItemNodeWrap">
          <Node :nodeList="null" :childrenList="data.children" :data="node" :belongConditionNodeData="data" :isMouseEnter="isMouseEnter"> </Node>
          <!-- 连接较短分支和分支整体右侧的水平线 -->
